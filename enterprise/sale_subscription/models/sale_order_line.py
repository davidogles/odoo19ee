...
class SaleOrderLine(models.Model):
    ...
    def _get_invoice_line_parameters(self):
        ...
        if is_upsell or not self.order_id.plan_id.billing_first_day or self.order_id.plan_id.billing_period_unit == 'week':
            ...
            # Align billing to calendar month: go to last day of start month, then +1 day to get next month's 1st
            next_date_1st = new_period_start + relativedelta(day=31, days=1)
        elif self.order_id.plan_id.billing_period_unit == 'year':
            # Align billing to calendar year: go to Dec 31 of start year, then +1 day to get next year's Jan 1st
            next_date_1st = new_period_start + relativedelta(month=12, day=31, days=1)
